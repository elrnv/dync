...
pub trait Elem: Any + Bytes + CloneFromBytes + DropBytes {}
impl<T> Elem for T where T: Any + Bytes + CloneFromBytes + DropBytes {}

type CloneFnType = unsafe fn(&[u8]) -> Box<[u8]>;
type CloneFromFnType = unsafe fn(&mut [u8], &[u8]);
type DropFnType = unsafe fn(&mut [u8]);

macro_rules! impl_fn_wrapper {
    (struct $fn:ident ( $fn_type:ident )) => {
        #[derive(Copy, Clone)]
        pub(crate) struct $fn (pub(crate) $fn_type);
        ...
        impl std::hash::Hash for $fn {
            fn hash<H: std::hash::Hasher>(&self, _: &mut H) {
                // Don't hash the function, since all CloneFns are considered equal anyway.
            }
        }
    }
}

impl_fn_wrapper!(struct CloneFn(CloneFnType));
impl_fn_wrapper!(struct CloneFromFn(CloneFromFnType));
impl_fn_wrapper!(struct DropFn(DropFnType));
